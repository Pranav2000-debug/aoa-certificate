...
function correctNameFormat(str) {
  ...
  return [capitalFirstName, capitalLastName].join(" ");
}
...
export async function generateCertificatePdf(memberData, options = {}) {
  ...
  const {
    ...
    positions = {
      ...
      issuedDate: { x: 657, y: 119, size: 13, maxWidth: 180 }, // bottom right
    },
    filenamePrefix = "",
  } = options;
  ...
  function isMeaningfulName(str) {
    ...
    if (!s) return false; // remove ALL non-alphanumeric chars (so "N*A", "N/A", " n a " -> "na")
  ...
  try {
    ...
    const arrayBuffer = await resp.arrayBuffer(); // 2) Load PDF document

    const pdfDoc = await PDFDocument.load(arrayBuffer);
    const pages = pdfDoc.getPages();
    const page = pages[0]; // 3) Embed font

    const font = await pdfDoc.embedFont(StandardFonts.Helvetica); // 4) Prepare values (preferring full names where available)
    ...
    const correctedOwnerName = correctNameFormat(ownerName);
    // Get the raw co-owner name to check it first
    const rawCoOwner = memberData.coOwnerName || memberData.coOwnerNameMasked || ""; // Helper: draw wrapped text
    ...
    if (isMeaningfulName(rawCoOwner)) {
      // Only format the name if it's meaningful to prevent errors.
      const correctedCoOwnerName = correctNameFormat(rawCoOwner);
      drawWrappedText(page, correctedCoOwnerName, positions.coOwnerName);
    }

    drawWrappedText(page, flatNumber, positions.flatNumber);
    drawWrappedText(page, tower, positions.tower);
    drawWrappedText(page, issuedDate, positions.issuedDate); // 6) Save and prepare download
    const pdfBytes = await pdfDoc.save();
    const blob = new Blob([pdfBytes], { type: "application/pdf" }); // Trigger download (also return blob)
